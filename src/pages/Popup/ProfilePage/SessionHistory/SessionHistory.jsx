...
class SessionHistory extends Component {
  ...
  constructor(props) {
    ...
    var sessionRef = db.collection('user').doc("yiw031@ucsd.edu").collection('sessions');
    if(this.props.user){
      sessionRef.get().then(function(querySnapshot){
        ...
        for(let i = 0; i < 7; i++){ 
          ...
          if(Day <= 0){
            Mon -= 1;
            if(["Jan", "Mar", "Mar", "Jul", "Aug", "Oct", "Dec"].includes(thisDay[1])){
          ...
          if (i == 0){
            ...
            if(Day >= 10)
              Date.push(Mon.toString() + "/" + Day.toString());
            else
            Date.push(Mon.toString() + "/0" + Day.toString())
          }
        }
        ...
        querySnapshot.forEach(function(doc){
          ...
          if(dayHistory.has(sessionDate) && doc.data().is_complete == true){
            dayHistory.set(sessionDate, dayHistory.get(sessionDate) + doc.data().session_length);
            dayNumber.set(sessionDate, dayNumber.get(sessionDate) + 1);
          }else if(!dayHistory.has(sessionDate) && doc.data().is_complete == true) {
